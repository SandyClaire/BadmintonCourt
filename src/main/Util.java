...
public class Util {
	...
	public static int valueCalculate(int startTime, int endTime, boolean isWeekday) {
		int value = 0;
	...
	public static boolean isInfoCorrect(String[] array) {
    	...
    	if (array[1].length() != 10 || array[2].length() != 11) {
    		System.out.println("3" + INVALID);
    		return false;
		}
    	if (!array[2].matches("^[0-9]{2}[:]{1}[0]{2}[~]{1}[0-9]{2}[:]{1}[0]{2}$")) {
    	...
    	String time = new StringBuilder(array[1]).append(" ").append(array[2].substring(0, 5)).toString();
    	...
    	if (date == null || (date.getTime() - System.currentTimeMillis() < 0)) {
    		System.out.println("7" + INVALID);
    		return false;
		}
    	if (!array[3].matches("^[ABCD]{1}$")) {
    		System.out.println("8" + INVALID);
    		return false;
		}
    	if (array.length == 5 && !array[4].equals("C")) {
    		System.out.println("9" + INVALID);
    		return false;		
		}
		return true;
	}
